...
| **借 `__dict__` / 凑 `co_filename` 冒充内部模块** | 🟢 已封：模块级帧比"该模块的模块级 code object 本身"，不再比任何可读字符串（第七轮） | 架构 |
| **付费进程里拆墙** | 🟢 已封：探针 + 注册表指纹 + 蜜罐 + `del sys.monitoring` 改成**所有模式都装**（第六轮 R6-02） | 架构 |
| **借内部帧当挡箭牌** | 🟢 已封：走栈遇到内部帧改成"跳过继续找"而不是"直接放行"（第七轮 R7-01/02/03） | 架构 |
| **到期判定的输入在客户端** | 🟢 已封：宽限期**签进租约载荷**；到期判定用可信时间（网络时间/高水位/本地取最大）（第七轮 R7-06） | 架构 |
| **明文通道上的 bearer 租约** | 🟢 已修（**换通道**而非补规则）：服务端上 TLS + 客户端 **cert pinning** → 同出口观察者只能盲目转发；自签证书的中间人被拒绝握手。见 `exploits/REPORT_ARCH_TLS.md` | 传输层 |
| 服务端状态文件里的 `refresh` 明文落盘 | 🔴 仍在：TLS 保护的是**通道**，不是**这台机器** —— 同 uid 读文件、改客户端仍是进程外问题 | 分发/主机 |
| **新线程根帧没有调用者** | 🟢 已封：入口帧（`f_back is None`）一律判用户代码，不再判"说不清"（第八轮 R8-01） | 架构 |
| **到期判定函数可替换** | 🟢 已封：时钟默认参数绑进 `LeaseKeeper`；可信时间拿不到时 fail-closed（返回 `inf`），不退化成可 patch 的 `time.time()`（第八轮 R8-02） | 架构 |
| **装配期性能** | 🟢 已修：装配期抑制开关把免费版启动 CPU 从 5.01s 压回 **1.05s**（第八轮 R8-04，上一轮修法的自伤） | 性能 |
| **吊销被重新激活抹掉 / 明文 bearer 租约** | 🟢 已封：`bind()` 不碰吊销列表（解除要 `/unrevoke` + 管理令牌）；`/renew` 要一次性刷新令牌（每次轮换） | 架构 |
